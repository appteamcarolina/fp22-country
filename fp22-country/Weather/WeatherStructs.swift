...
extension WeeklyForecastResponse {
    ...
    static let example = WeeklyForecastResponse(lat: 51.50998, lon: -0.1337, timezone: "Europe/London", daily: [DailyForecast.example, DailyForecast.example, DailyForecast.example])
    static var example2: WeeklyForecastResponse {
        var decoded: WeeklyForecastResponse? = nil
        do {
            decoded = try JSONDecoder().decode(WeeklyForecastResponse.self, from: example2String.data(using: .utf8)!)
        }
        catch {
...
extension DailyForecast {
    static let example = DailyForecast(dt: Date().timeIntervalSince1970, sunrise: Date().timeIntervalSince1970, sunset: Date().timeIntervalSince1970, temp: TempForecast(day: 25.0, min: 25.0, max: 25.0, night: 25.0, eve: 25.0, morn: 25.0), weather: [WeatherForecast(main: "Sunny", description: "Sunny")])
}
